 ...
 */
function AES_Init() {
    AES.AES_Sbox_Inv = new Array(256);
    for (let i = 0; i < 256; i++)
        AES.AES_Sbox_Inv[AES.AES_Sbox[i]] = i;

    AES.AES_ShiftRowTab_Inv = new Array(16);
    for (let i = 0; i < 16; i++)
        AES.AES_ShiftRowTab_Inv[AES.AES_ShiftRowTab[i]] = i;

    AES.AES_xtime = new Array(256);
    for (let i = 0; i < 128; i++) {
        AES.AES_xtime[i] = i << 1;
        AES.AES_xtime[128 + i] = (i << 1) ^ 0x1b;
    }
}
...
function AES_Done() {
    delete AES.AES_Sbox_Inv;
    delete AES.AES_ShiftRowTab_Inv;
    delete AES.AES_xtime;
}
...
/* No longer use.
 ...
 if (i % kl == 0) {
 temp = new Array(AES.AES_Sbox[temp[1]] ^ Rcon, AES.AES_Sbox[temp[2]],
 AES.AES_Sbox[temp[3]], AES.AES_Sbox[temp[0]]);
 if ((Rcon <<= 1) >= 256)
 Rcon ^= 0x11b;
 } else if ((kl > 24) && (i % kl == 16))
 temp = new Array(AES.AES_Sbox[temp[0]], AES.AES_Sbox[temp[1]],
 AES.AES_Sbox[temp[2]], AES.AES_Sbox[temp[3]]);
 for(var j = 0; j < 4; j++)
...
function AES_Encrypt(block, key) {
    ...
    for (let i = 16; i < l - 16; i += 16) {
        AES_SubBytes(block, AES.AES_Sbox);
        AES_ShiftRows(block, AES.AES_ShiftRowTab);
...
function AES_Decrypt(block, key) {
    ...
    AES_ShiftRows(block, AES.AES_ShiftRowTab_Inv);
    AES_SubBytes(block, AES.AES_Sbox_Inv);
...
AES.AES_Sbox = [99, 124, 119, 123, 242, 107, 111, 197, 48, 1, 103, 43, 254, 215, 171,
...
AES.AES_ShiftRowTab = [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11];
...
function AES_MixColumns(state) {
    for (let i = 0; i < 16; i += 4) {
        const s0 = state[i], s1 = state[i + 1];
        const s2 = state[i + 2], s3 = state[i + 3];
        const h = s0 ^ s1 ^ s2 ^ s3;
        state[i] ^= h ^ AES.AES_xtime[s0 ^ s1];
        state[i + 1] ^= h ^ AES.AES_xtime[s1 ^ s2];
        state[i + 2] ^= h ^ AES.AES_xtime[s2 ^ s3];
        state[i + 3] ^= h ^ AES.AES_xtime[s3 ^ s0];
    }
}
...
function AES_MixColumns_Inv(state) {
    for (let i = 0; i < 16; i += 4) {
        ...
        const s2 = state[i + 2], s3 = state[i + 3];
        const h = s0 ^ s1 ^ s2 ^ s3;
        const xh = AES.AES_xtime[h];
        const h1 = AES.AES_xtime[AES.AES_xtime[xh ^ s0 ^ s2]] ^ h;
        const h2 = AES.AES_xtime[AES.AES_xtime[xh ^ s1 ^ s3]] ^ h;
        state[i] ^= h1 ^ AES.AES_xtime[s0 ^ s1];
        state[i + 1] ^= h2 ^ AES.AES_xtime[s1 ^ s2];
        state[i + 2] ^= h1 ^ AES.AES_xtime[s2 ^ s3];
        state[i + 3] ^= h2 ^ AES.AES_xtime[s3 ^ s0];
    }
}
 ...
 * @return {string}
 */
exports.AES_Encrypt128 = (passwd, PrivateKey) => {
    ...
    let output = array2hexstr(output_byte);
    AES_Done();
    return output;
};
...
function AES_Encrypt_SeqNo(passwd, key) {
    ...
    const private_key_byte = hexstr2array(key, 32);
    const passwd_byte = hexstr2array(passwd_hex, 64);
    const output_byte = new Array(64);
    AES_Init();
...
function AES_Decrypt128(encrypted) {
    ...
    const private_key_byte = hexstr2array(PrivateKey, 32);
    const encrypted_byte = hexstr2array(encrypted, 64);
    const output_byte = new Array(64);
    AES_Init();
